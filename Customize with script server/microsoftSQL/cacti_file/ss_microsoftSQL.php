...
if (!isset($called_by_script_server)) {
	...
	print call_user_func_array('ss_microsoftSQL', $_SERVER['argv']);
} else {
	include_once(dirname(__FILE__) . '/../lib/snmp.php');
}

function ss_microsoftSQL($hostname = '', $host_id = 0, $snmp_auth = '', $cmd = 'index', $arg1 = '', $arg2 = '') {
	...
	$config = simplexml_load_file($PATH_TO_CONFIG.'microsoftSQL_config.xml');
	$host = $config->dbSetting->host;
    $user = $config->dbSetting->user;
    $pwd = $config->dbSetting->pwd;
    $database = $config->dbSetting->database;
    $table = $config->dbSetting->table;
